<div align="center">
  <img src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="520" alt="Developer coding animation" />

  <h3>Minjae's GitHub</h3>
  <p>
    Java와 Spring 생태계를 중심으로<br />
    백엔드의 동작 원리를 작은 코드로 확인하고 기록합니다.
  </p>

  <a href="https://github.com/minjae2674">
    <img src="https://img.shields.io/badge/GitHub-minjae2674-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub profile" />
  </a>
</div>

---

## Tech Stack

### Backend

<p>
  <img src="https://img.shields.io/badge/Java_17-ED8B00?style=flat-square&logo=openjdk&logoColor=white" alt="Java 17" />
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=springboot&logoColor=white" alt="Spring Boot" />
  <img src="https://img.shields.io/badge/Spring_Security-6DB33F?style=flat-square&logo=springsecurity&logoColor=white" alt="Spring Security" />
  <img src="https://img.shields.io/badge/Spring_Framework-6DB33F?style=flat-square&logo=spring&logoColor=white" alt="Spring Framework" />
  <img src="https://img.shields.io/badge/JSP-007396?style=flat-square&logo=openjdk&logoColor=white" alt="JSP" />
  <img src="https://img.shields.io/badge/MyBatis-DC382D?style=flat-square&logoColor=white" alt="MyBatis" />
  <img src="https://img.shields.io/badge/Spring_Batch-6DB33F?style=flat-square&logo=spring&logoColor=white" alt="Spring Batch" />
  <img src="https://img.shields.io/badge/Spring_AI-6DB33F?style=flat-square&logo=spring&logoColor=white" alt="Spring AI" />
  <img src="https://img.shields.io/badge/JPA_%2F_Hibernate-59666C?style=flat-square&logo=hibernate&logoColor=white" alt="JPA and Hibernate" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" alt="MySQL" />
</p>

### Mobile & Frontend

<p>
  <img src="https://img.shields.io/badge/Kotlin-7F52FF?style=flat-square&logo=kotlin&logoColor=white" alt="Kotlin" />
  <img src="https://img.shields.io/badge/Android-3DDC84?style=flat-square&logo=android&logoColor=white" alt="Android" />
  <img src="https://img.shields.io/badge/Jetpack_Compose-4285F4?style=flat-square&logo=jetpackcompose&logoColor=white" alt="Jetpack Compose" />
  <img src="https://img.shields.io/badge/Vue.js-4FC08D?style=flat-square&logo=vuedotjs&logoColor=white" alt="Vue.js" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=111111" alt="JavaScript" />
</p>

### Infrastructure & Collaboration

<p>
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonwebservices&logoColor=white" alt="AWS" />
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" alt="Git" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub" />
  <img src="https://img.shields.io/badge/Jira-0052CC?style=flat-square&logo=jira&logoColor=white" alt="Jira" />
  <img src="https://img.shields.io/badge/Gradle-02303A?style=flat-square&logo=gradle&logoColor=white" alt="Gradle" />
  <img src="https://img.shields.io/badge/Maven-C71A36?style=flat-square&logo=apachemaven&logoColor=white" alt="Maven" />
</p>

---

## Certifications & Qualifications

- 정보처리기사 · 빅데이터분석기사 · SQLD · ADsP
- TOEIC Speaking 150 · 한국사능력검정시험 2급 · 컴퓨터활용능력 2급

---

## Learning Repositories

| Repository | What I explored |
| --- | --- |
| **[Quokkive](https://github.com/minjae2674/Quokkive)** | 스크린샷과 링크를 온디바이스 AI로 분석해 아카이브·검색·일정·할 일로 연결한 Android 팀 프로젝트 |
| **[codex_ssafyHome_test](https://github.com/minjae2674/codex_ssafyHome_test)** | 지도 기반 아파트 검색과 JWT 인증 흐름을 Java와 Vue로 연결한 풀스택 실습 |
| **[SpringAi](https://github.com/minjae2674/SpringAi)** | Spring AI·OpenAI 모델 연동, JDBC 채팅 메모리, JPA·MySQL 실습 |
| **[SpringJWT](https://github.com/minjae2674/SpringJWT)** | Spring Security와 JJWT를 활용한 인증 흐름 구현 |
| **[SpringBatch](https://github.com/minjae2674/SpringBatch)** | Spring Batch·JPA·JDBC를 활용한 배치 처리 학습 |
| **[aichallenge](https://github.com/minjae2674/aichallenge)** | Colab 기반 AI Challenge 베이스라인 실험 노트북 |

---

## Currently Exploring

- Spring Security와 JWT를 활용한 인증·인가 흐름
- Spring Batch를 활용한 데이터 처리
- Spring AI와 LLM을 연결한 백엔드 기능
- Java 백엔드와 Vue 프론트엔드 사이의 API 연동

---

## What I Care About

- 인증, 데이터 저장, 배치 처리가 자연스럽게 이어지는 백엔드 흐름
- 새로운 기술을 작은 실습으로 나눠 실제 동작까지 확인하는 학습 방식
- 다른 사람이 다시 따라갈 수 있도록 남기는 코드와 기록

